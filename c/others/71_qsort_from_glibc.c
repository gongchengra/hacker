...
* Purpose : https://github.com/lattera/glibc/blob/master/stdlib/qsort.c
...
int main(int argc, char **argv) {
    int arr[SIZE];
    randData(arr, SIZE);
    _quicksort(arr, SIZE, sizeof(int), cmp);
    printArray(arr, SIZE);
#if 0
    printf("input strings:");
    char str[SIZE][10 * SIZE];
    for (int i = 0; i < SIZE; i++) {
        fgets(str[i], sizeof(str[i]), stdin);
    }
    _quicksort(str, SIZE, 10 * SIZE, scmp);
    printf("\n");
    for (int i = 0; i < SIZE; i++) {
        printf("%s", str[i]);
    }
